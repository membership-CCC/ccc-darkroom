...
def build_sheet(roll_dir: Path, out_path: Path | None = None) -> Path | None:
    ...
    # Route-safety holds render into _hold/, so a roll can legitimately have
    # no top-level web/ folder at all. Look in both, held frames included —
    # the whole point of the sheet is to review everything that came in.
    web_dirs = [roll_dir / "web", roll_dir / "_hold" / "web"]
    if not rows or not any(d.exists() for d in web_dirs):
        # --all walks every folder under Photos/, including unrelated ones.
        # Only complain about folders that look like rolls; anything without a
        # manifest was never ours, and warning about it every 15 minutes for
        # the life of the install is just noise in the log.
        if rows:
            LOG.warning("%s: manifest present but no web folder", roll_dir.name)
        else:
            LOG.debug("%s: not a processed roll, skipping", roll_dir.name)
        return None
    ...
    thumbs = []
    for row in rows:
        p = next((d / f"{row['stem']}_web.jpg" for d in web_dirs
                  if (d / f"{row['stem']}_web.jpg").exists()), None)
        if p is None:
            continue
